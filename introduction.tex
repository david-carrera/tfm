...
This is the case not only for English but for every tested human language. \cite{Mehri2017a}
...
Its formulation is Equation \eqref{eq:zipf_law}, the relationship between a word's frequency and frequency rank follows a power law.
...
\begin{equation}
  \label{eq:meaning-frequency-law}
  \mu \propto f^\delta.
\end{equation}
...
\begin{equation}
  \label{eq:meaning-distribution-law}
  \mu \propto i^{-\gamma}
\end{equation}
...
\begin{figure}
  ...
  \caption{
    ...
    Each of the patterns of the columns in the graph correspond with a time period.
...
\input{tables/summary-models-laws}
...
Mutual information
\begin{equation}
  \label{eq:definition-mutual-information}
  I(X,Y) = H(X) + H(Y) - H(X,Y),
\end{equation}
...
\input{tables/summary-models-complexity}
...
As seen at the start of this chapter and in \cite{Ferrer2018a}, there is a relationship between the exponents of the Zipf's word-frequency and meaning-frequency laws.
...
\redtxt{It is hypothesized that the initial condition of the optimization process plays an important role in determining the outcome.
  We use several configurations that are known to be maxima or minima of the cost function in order to explore the effect that the initial condition of the graph has in the final outcome.}

\redtxt{The four initial conditions explored are a random bipartite graph, a single link between one word and one meaning, a one to one bijection between words and meanings and a complete bipartite graph.
They have been chosen since they result in extreme values of some interesting metrics of the graph.
The four initial conditions used are summarized in Table \ref{tab:summary-initial-condition} as well as their relationship to some important metrics of the graph.}

\input{tables/summary-initial-condition}

\redtxt{Linguistic laws have been previously recovered in the transition between a referentially useless system and an indexical reference system. \cite{Ferrer2003a}
  A referentially useless system is one where referentially useless words dominate.
  We define a word $s_i$ as referentially useless if it is connected to at least one meaning and for each meaning $r_j$ it is connected to,
  \begin{equation}
    \label{eq:referentially-useless-word}
    p(s_i,r_j) \leq p(s_i) \cdot p(r_j).
  \end{equation}
  We hypothezise that referentially useless words are common during the referentially useless phase (\lambdaZeroToHalf{}) and disappear for greater values of $\lambda$.
}

\redtxt{Another interesting metric is the largest connected component of the graph.
Almost connectedness of the word-meaning graph has been hypothesized to be the mathematical condition necessary for the appearance of syntax and symbolic reference. \cite{Ferrer2005b} \cite{Ferrer2006a}
We hypothesize that a large proportion of the nodes of the graph will belong to the same connected component when language laws can be recovered.}

The reason for introducing the $\phi$ parameter is that when $\phi=1$ the models should be able to reproduce Zipf's meaning-frequency law according to \cite{Ferrer2018a}.
We expect to see that the meaning frequency law will appear then.

Another of the questions that we seek to answer is whether local minima exist, as argued in previous papers \cite{Ferrer2017a}.
